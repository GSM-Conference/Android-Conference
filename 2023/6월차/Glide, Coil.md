...
## 결론
위 테스트 데이터를 보면 아래 상황일 때, 각 라이브러리의 이점이 있는 것으로 판단된다.

### Glide
- 반복되는 이미지가 많아서 캐시 데이터가 많이 필요할 때
- 원본 이미지 사이즈와 ImageView 사이즈 간의 편차가 클 때
- Warm Start가 자주 일어나는 환경일 때
- 순간적으로 많은 이미지가 로딩됐을 때, 메모리 사용량 편차가 크면 안되는 상황일 때

<br>

### Coil
- 메모리 사용량에 여유가 있고, 다양한 이미지를 사용해 캐시가 동작하기 어려울 때
- Custome Image Transform을 사용하는 부분이 많을 때

Coil은 아직 RC버전이라, 이후 안정화된 버전 출시 및 다양한 기능이 업데이트 됐을 때 다시 테스트를 해봐야할 것 같다.